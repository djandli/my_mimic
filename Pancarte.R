...
#
source( "/Users/hedjoudje/Documents/Recherche/MIMIC-III/SCRIPT_MIMIC/selection_pancreatite_aigue.R")
...
url<-"https://docs.google.com/spreadsheets/d/18f_HCamGi8AGPdeoT1B2bz2ml05sjxT92UHXxlV80UM/edit?usp=sharing"
...
#for (i in 1:50){
#for (i in c(1:37,39:length(unique(temp5$hadm_id)))){
for (i in 70:length(unique(temp5$hadm_id))){ 
#for (i in 1:10){  
#i<-69
  print(i)
  Numero<-i
...
## Liver enzymes
  ...
  {if(sum(temp7$VARIABLE == "Amylase",na.rm = TRUE) >0 )
  ...
  {if(sum(temp7$VARIABLE == "Lipase",na.rm = TRUE) >0 )
  ...
  {if(sum(temp7$VARIABLE == "Asparate Aminotransferase (AST)" ,na.rm = TRUE) >0 )
...
{if(sum(temp7$VARIABLE == "Alanine Aminotransferase (ALT)" ,na.rm = TRUE) >0 )
 ...
 {if(sum(temp7$VARIABLE == "Alkaline Phosphatase" ,na.rm = TRUE) >0 )
  ...
  {if(sum(temp7$VARIABLE == "Bilirubin T" ,na.rm = TRUE) >0 )
...
## Add noteeevents
  ...
  {if(sum(ntes$description %in%c("CHEST (PORTABLE AP)" ,"CHEST (SINGLE VIEW)" ) ,na.rm = TRUE) >0 )  
...
max(max(ntes$TEMPS, na.rm = TRUE), max(temp6$TEMPS, na.rm = TRUE)) /10->largeur
png(file = paste("./Planche/", unique(temp5$hadm_id)[Numero], ".png"), height = 10, width = largeur, 
     res = 300, units = "in" )
print(g)
dev.off()
