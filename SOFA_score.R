...
sql <- paste("select * from mimic3.sofa where  hadm_id in (", ids, ") ", sep=" " )
data <- dbGetQuery(con, sql)



write.csv(x = data , file = "./resultats/sofa.csv", row.names = FALSE)

print("##################################")
print(" Le fichier sofa.csv vient d'être créé")
print("##################################")
