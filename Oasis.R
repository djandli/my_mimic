...
sql <- paste("select * from mimic3.oasis where  hadm_id in (", ids, ") ", sep=" " )
data <- dbGetQuery(con, sql)



write.csv(x = data , file = "./resultats/oasis.csv", row.names = FALSE)

print("##################################")
print(" Le fichier oasis.csv vient d'être créé")
print("##################################")
